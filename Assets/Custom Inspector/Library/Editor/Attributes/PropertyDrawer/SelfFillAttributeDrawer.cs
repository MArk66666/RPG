...
namespace CustomInspector.Editor
{
    ...
    public class SelfFillAttributeDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent _)
        {
            ...
            {
                DrawProperties.DrawFieldWithMessage(position, label, property, $"SelfFill: type {property.propertyType} not supported. Use this attribute on objects (interfaces with RequireType or components)", MessageType.Error, disabled: true);
            }
            else
            {
                ...
                if (property.objectReferenceValue == null)
                {
                    ...
                    else if(requiredInterface != null)
                    {
                        ...
                        if (property.objectReferenceValue == null)
                        {
                            DrawProperties.DrawFieldWithMessage(position, label, property, $"SelfFill: no component with interface '{requiredInterface.requiredType}' on this gameObject", MessageType.Error, disabled: true);
                            return;
                        }
                    }
                    ...
                    {
                        DrawProperties.DrawFieldWithMessage(position, label, property, $"SelfFill: type '{fieldInfo.FieldType}' not supported. Use component-types (no Assets) or interfaces (with the [RequireType]-attribute", MessageType.Error, disabled: true);
                        return;
                    }
                }

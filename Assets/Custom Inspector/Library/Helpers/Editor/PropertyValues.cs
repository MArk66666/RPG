    ...
    public static class PropertyValues
    {
        ...
        public static IEnumerable<SerializedProperty> GetAllPropertys(this IFindProperties finder, bool alsoFromBase)
        {
            ...
            else
            {
                ...
                IEnumerable<FieldInfo> privates;
                IEnumerable<FieldInfo> publics;
                if (alsoFromBase)
                {
                    List<Type> typeAndBases = new();
                    {
                        Type t = type;
                        Debug.Assert(t != null, "Type of property not found");
                        do
                        {
                            typeAndBases.Add(t);
                            t = t.BaseType;
                        }
                        while (t != typeof(System.Object)); //all classes are derived from Object in the end
                    }
                    privates = typeAndBases.Select(x => x.GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly)
                                                                                .Where(x => Attribute.IsDefined(x, typeof(SerializeField))))
                                                                  .SelectMany(x => x);
                    publics = type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.FlattenHierarchy)
                                        .Where(x => !Attribute.IsDefined(x, typeof(NonSerializedAttribute)));
                }
                else
                {
                    privates = type.GetFields(BindingFlags.Instance | BindingFlags.NonPublic)
                                        .Where(x => Attribute.IsDefined(x, typeof(SerializeField)));
                    publics = type.GetFields(BindingFlags.Instance | BindingFlags.Public)
                                        .Where(x => !Attribute.IsDefined(x, typeof(NonSerializedAttribute)));
                }
                IEnumerable<FieldInfo> fields = privates.Concat(publics);
                //No null fields
                ...
                //No duplicates
                {
                    var duplicates = fields.GroupBy(x => x.Name)
                                           .Where(x => x.Count() > 1);
                    Debug.Assert(!duplicates.Any(), "Fields duplicates found: " + string.Join(", ", duplicates.Select(x => $"{x.Count()} times \"{x.First().Name}\"" +
                                                    $"\non type {type}:" +
                                                    "\nPrivates: '" + string.Join(", ", privates.Select(x => x.Name)) + "'" + 
                                                    "\nPublics: '" + string.Join(", ", publics.Select(x => x.Name)) + "'"
                                                    )));
                }
                //convert
        ...
        /// </summary>
        public static IEnumerable<SerializedProperty> GetAllPropertys(this SerializedObject obj)
        {
            FieldInfo[] fields = obj.targetObject.GetType().GetFields(defaultBindingFlags);
            return fields.Where(_ => !Attribute.IsDefined(_, typeof(NonSerializedAttribute))).Select(_ => obj.FindProperty(_.Name)).Where(_ => _ != null);
        }
        /// <summary>
        ...
        public static IEnumerable<SerializedProperty> GetAllVisiblePropertys(this SerializedObject obj)
        => obj.GetAllPropertys().OnlyVisible();
        /// <summary>
